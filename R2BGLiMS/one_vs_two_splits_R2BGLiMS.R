...
dim(df1[df1$counter_sim==1,])


keys_per_split1 <- df1 %>% filter(counter_sim == 1, split==1) %>% dplyr::select(key_model)
keys_per_split2 <- df1 %>% filter(counter_sim == 1, split==2) %>% dplyr::select(key_model)
common_keys <- intersect(keys_per_split1, keys_per_split2)
...
colMeans(res_simple_comp$M1$betasamples[[2]])
